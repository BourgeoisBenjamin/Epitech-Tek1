    ...
    sfTexture *texture, sfSprite *sprite)
{
    ...
    while (sfRenderWindow_isOpen(window)) {
        ...
        generate_cube_rotation(fb, cube, center1);
        generate_cube_rotation(fb, cube, center2);
        generate_cube_rotation(fb, cube, center3);
...
void generate_cube_rotation(framebuffer_t *fb, t_cube *cube, sfVector2i center)
{
    ...
    for (int i = 0; i < 8; i++) {
        cube[i].x = (cos(angle) * cube[i].x) - (sin(angle) * cube[i].y);
        cube[i].y = (sin(angle) * cube[i].x) + (cos(angle) * cube[i].y);
