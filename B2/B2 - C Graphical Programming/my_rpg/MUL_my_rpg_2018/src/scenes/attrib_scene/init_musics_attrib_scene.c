...
{
    t_music *background = create_music("ressources/soundtrack/foggy.ogg",
    50, 1, BACKGROUND_M);
...
t_music *init_musics_attrib_scene(void)
{
    ...
    if ((list_music = create_background_music_attrib_scene(list_music)) == NULL)
        return (NULL);
    return (list_music);
}
