...
t_game_object *create_background_htp_scene(t_game_object *list_obj)
{
    ...
    pos, rect, 1920);
    t_game_object *htp = create_object("ressources/back/htp.png",
    pos, rect, 1920);
    ...
    if ((list_obj = add_object(list_obj, background)) == NULL)
        return (NULL);
    htp->type = BACKGROUND_O;
    if ((list_obj = add_object(list_obj, htp)) == NULL)
        return (NULL);
    return (list_obj);
}
...
t_game_object *init_objs_htp_scene(void)
{
    ...
    if ((list_obj = create_background_htp_scene(list_obj)) == NULL)
        return (NULL);
    return (list_obj);
}
